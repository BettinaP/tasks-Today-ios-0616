...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    EventDetailViewController *destinationVC =[segue destinationViewController];
    
    if ([segue.identifier isEqualToString:@"lastYear"]){
        NSArray *lastYearEvents = [self getLastYearsEvents];
        
        destinationVC.yearlyEvents = lastYearEvents;
    
    }else if ([segue.identifier isEqualToString:@"nextYear"]) {
        NSArray *nextYearEvents =[self getNextYearsEvents];
        
        destinationVC.yearlyEvents = nextYearEvents;
